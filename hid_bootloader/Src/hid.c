...
// bit position of USB_COUNTn_RX NUM BLOCK
#define POS_USB_COUNTn_RX_NUMBLOCK 10
...
static const uint8_t Command[] __ALIGNED(4) = {'B', 'T', 'L', 'D', 'C', 'M', 'D', 2};

/* Flash page buffer */
static uint8_t PageData[PAGE_SIZE] __ALIGNED(4);
...
/* USB Descriptors */
static const uint8_t USB_DeviceDescriptor[] __ALIGNED(4) = {
	0x12,			// bLength
...
static const uint8_t USB_ConfigurationDescriptor[] __ALIGNED(4) = {
	0x09,			// bLength
...
static const uint8_t USB_ReportDescriptor[32] __ALIGNED(4) = {
...
static const uint8_t USB_LangIDStringDescriptor[] __ALIGNED(4)  = {
	0x04,			// bLength
	0x03,			// bDescriptorType (String)
	0x09, 0x04		// English (United States)
};

static const uint8_t USB_VendorStringDescriptor[] __ALIGNED(4) = {
	0x22,			// bLength
	0x03,			// bDescriptorType (String)
...
static const uint8_t USB_ProductStringDescriptor[] __ALIGNED(4) = {
	0x2C,			// bLength
	0x03,			// bDescriptorType (String)
...
void USB_Reset(void)
{
	...
	BTABLE_ADDR_FROM_OFFSET(ENDP0, BTABLE_OFFSET)[USB_COUNTn_RX] = 1 << POS_USB_COUNTn_RX_NUMBLOCK | 1<<15;
...
void USB_EPHandler(uint16_t status)
{
	...
	if (READ_BIT(endpoint_status, EP_CTR_RX)) {
		...
		if (endpoint == 0) {
			...
			/* If control endpoint */
			if (READ_BIT(endpoint_status, USB_EP_SETUP)) {
