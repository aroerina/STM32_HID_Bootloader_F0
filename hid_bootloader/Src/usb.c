...
USB_RxTxBuf_t RxTxBuffer[MAX_EP_NUM] __ALIGNED(4);
...
{
	volatile uint16_t *btable = BTABLE_ADDR(endpoint);
	uint16_t count = RxTxBuffer[endpoint].RXL = (btable[USB_COUNTn_RX] & 0x3ff) / 2;
	uint16_t *address = (uint16_t *) (PMAAddr + btable[USB_ADDRn_RX]);
